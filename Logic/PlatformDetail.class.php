...
class PlatformDetail extends BaseLogic
{
    public function data($stime='',$etime=''){
        ...
        $reg_sum = $this->get_reg_sum1($stime,$etime);
        ...
        //当日系统输赢,玩家总金币
        $sys_user_log = $this->get_system_user_log($stime,$etime);
        ...
        foreach ($data as $k=>$v){
            ...
            //当日系统输赢
            $data[$k]['sys_win'] = $sys_user_log[$v['date']]['systemwin']?$sys_user_log[$v['date']]['systemwin']:0;
            //当日玩家总金币
            $data[$k]['user_gold'] = $sys_user_log[$v['date']]['usergold']?$sys_user_log[$v['date']]['usergold']:0;
            //vip总金币
            $data[$k]['vip_gold'] = $sys_user_log[$v['date']]['vipgold']?$sys_user_log[$v['date']]['vipgold']:0;
...
//        $m1 =  M('ScoreChangeDetail','','DB_RECOED');
    ...
    public function get_system_user_log($stime='',$etime=''){
        $m = M('SystemUserLog','','RDDB_USER');
        $map['LogTime'] = array('between',$stime.','.$etime);
        $data = $m->where($map)->field('LogTime,UserGold,SystemWin,VipGold')->select();
        $data = $this->changeKeys($data, 'logtime');
        return $data;
    }

    public function get_today_room_tax($stime='',$etime=''){
//        $m = M('AA_Log_GameRoomRevenue','','DB_SERVER');
        $m = M('AA_Log_GameRoomRevenue','','RDDB_SERVER');
        $map['ChangeDate'] = array('between',$stime.','.$etime);
...
//        $m = M('RecordUserLeave','','DB_RECOED');
        $m = M('RecordUserLeave','','RDDB_RECOED');
    ...
    public function get_reg_sum1($stime='',$etime=''){
        //$m =  M('AA_ZZ_Log_Register');
        $m =  M('AccountsInfo','','RDDB_USER');

        if($stime!=''&& $etime!=''){
            $map['UserID'] = array('gt',10000);
            $map['RegisterDate'] = array('between',$stime.','.$etime);
            $log_data = $m->where($map)->field('CONVERT(varchar(10), RegisterDate,21) as daytime,count(UserID) as total')->group('CONVERT(varchar(10), RegisterDate,21)')->select();

        }else{

            $log_data = $m->field('CONVERT(varchar(10), RegisterDate,21) as daytime,count(UserID) as total')->group('CONVERT(varchar(10), RegisterDate,21)')->select();
        }
            ...
            if($isCountScore){
                ...
                if($type == 'R'){
				 ...
				 if($type == 'R'){
						...
						$log_data = $m1->where($map)
                    ->field('CONVERT(varchar(10), ChangeDate,21) as daytime,count(DISTINCT(UserID)) as total')
                    ->group('CONVERT(varchar(10), ChangeDate,21)')
                    ->select();
				 }else{
					 $log_data = $m1->where($map)
...
//        $m4 = M('GameScoreLocker','','DB_TREASURE');
        ...
        $m = M('AccountsInfo','','RDDB_USER');
        $m1 = M('RecordUserEnter','','RDDB_RECOED');
        $m2= M('RecordUserLeave','','RDDB_RECOED');
        $m3= M('onlineTotal','','RDDB_USER');
        $m4 = M('GameScoreLocker','','RDDB_TREASURE');
