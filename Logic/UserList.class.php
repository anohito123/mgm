...
class UserList extends BaseLogic
{
	...
	public function  deal_data($uid = '',$stime='',$etime='',$type = 'R',$sort,$page_count){
        ...
        $map_null['ChangeDate'] = array('between',$today.','.$tomorrow);


        if($m1->where($map_null)->where('UserID!=UserId2')->select()){
            if($stime==''||$etime==''){
                $stime = $today;
                $etime = $tomorrow;
            }
...
//        $m3 = M('GameRoomItem','','DB_SERVER');
        ...
        foreach ($users as $k=>$v){
            ...
            $users[$k]['received_v'] =$users[$k]['received_sum'] - $users[$k]['received_p'];
