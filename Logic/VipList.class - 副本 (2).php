...
class VipList extends BaseLogic{
    ...
    public function vip_data1($stime='',$etime='', $type='player',$page_count){
        ...
        //总赠送
        $sum_present = $this->vip_pr($vips,$type,'P',date("Y-m-d",strtotime("-1 year")),$etime);

        //总赠送笔数
        $sum_present_times = $this->vip_pr($vips,$type,'P',date("Y-m-d",strtotime("-1 year")),$etime,'times');


        //总赠送人数
        $sum_present_person = $this->vip_pr($vips,$type,'P',date("Y-m-d",strtotime("-1 year")),$etime,'person');

        //总接收
        $sum_received = $this->vip_pr($vips,$type,'R',date("Y-m-d",strtotime("-1 year")),$etime);


        //总接收笔数
        $sum_received_times = $this->vip_pr($vips,$type,'R',date("Y-m-d",strtotime("-1 year")),$etime,'times');

        //总接收人数
        $sum_received_person = $this->vip_pr($vips,$type,'R',date("Y-m-d",strtotime("-1 year")),$etime,'person');

        //新加字段------------------------------------

        //近一周总赠送
        $sum_p_7 = $this->vip_pr($vips,$type,'P',date("Y-m-d",strtotime("-7 day")),$etime);

        //近一周总接收
        $sum_r_7 = $this->vip_pr($vips,$type,'R',date("Y-m-d",strtotime("-7 day")),$etime);


        //近一周赠送笔数
        $sum_p_times_week = $this->vip_pr($vips,$type,'P',date("Y-m-d",strtotime("-7 day")),$etime,'times');

        //近一周接收笔数
        $sum_r_times_week = $this->vip_pr($vips,$type,'R',date("Y-m-d",strtotime("-7 day")),$etime,'times');

        //上一周总赠送
        $sum_p_14 = $this->vip_pr($vips,$type,'P',date("Y-m-d",strtotime("-14 day")),date("Y-m-d",strtotime("-7 day")),$etime);

        //上一周总接收
        $sum_r_14 = $this->vip_pr($vips,$type,'R',date("Y-m-d",strtotime("-14 day")),date("Y-m-d",strtotime("-7 day")),$etime);

        //上一周赠送笔数
        $sum_p_times_last_week = $this->vip_pr($vips,$type,'P',date("Y-m-d",strtotime("-14 day")),date("Y-m-d",strtotime("-7 day")),'times');

        //上一周接收笔数
        $sum_r_times_last_week = $this->vip_pr($vips,$type,'R',date("Y-m-d",strtotime("-14 day")),date("Y-m-d",strtotime("-7 day")),'times');
